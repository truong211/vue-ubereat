...
// A simple class that mimics a Sequelize model but actually uses our direct SQL connection
class FAQ {
  constructor(data = {}) {
  ...
  async save() {
    // If the FAQ has an ID, it's an update, otherwise it's a create
    if (this.id) {
      const { id, ...data } = this;
      ...
      const sql = `UPDATE faqs SET ${setClauses.join(', ')} WHERE id = ?`;
      ...
      const sql = `INSERT INTO faqs (${columns}) VALUES (${placeholders})`;
  ...
  static async findAll(options = {}) {
    ...
    return results.map(row => new FAQ(row));
  }

  static async findByPk(id) {
    const sql = 'SELECT * FROM faqs WHERE id = ? LIMIT 1';
    const results = await query(sql, [id]);
    
    if (results.length === 0) return null;
    return new FAQ(results[0]);
  }
  ...
  static async update(data, options = {}) {
    ...
    let sql = `UPDATE faqs SET ${setClauses.join(', ')}`;
