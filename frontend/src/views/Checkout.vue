  ...
  methods: {
    ...
    async loadData() {
      try {
        ...
        this.$toast.error('Không thể tải dữ liệu thanh toán. Vui lòng thử lại.');
      } finally {
    ...
    async saveAddress() {
      ...
      try {
        ...
        const result = await this.saveAddressAction(addressData);
        return result;
      } catch (error) {
        console.error('Failed to save address:', error);
        this.$toast.error('Không thể lưu địa chỉ. Vui lòng thử lại sau.');
        return null;
      }
    },
    ...
    async placeOrder() {
      ...
      try {
        ...
        if (this.selectedPaymentMethod.type === 'cash') {
          ...
          this.$router.push(`/orders/${orderResult.id}`);
        } else {
        ...
        this.$toast.error('Không thể đặt hàng. Vui lòng thử lại sau.');
      } finally {
